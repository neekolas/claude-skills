...
export const initCommand = new Command("init")
  .description(
    "Initialize a new job (creates folder structure, job.json, empty task graph)",
  )
  .requiredOption("--name <name>", "Job name (will be sanitized)")
  .requiredOption(
    "--architecture <paths...>",
    "Path(s) to architecture/spec document(s)",
  )
  .option("--project-root <path>", "Project root directory", process.cwd())
  .action(async (opts) => {
    const projectRoot: string = opts.projectRoot;
    const { name } = await initJob(opts.name, opts.architecture, projectRoot);
    ...
    if (!branchResult.ok) {
      console.error(
        `Failed to create branch ${branchName}: ${branchResult.stderr}`,
      );
      process.exit(1);
    }
